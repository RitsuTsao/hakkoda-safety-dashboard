...
- Notification Layer v1:
  - `scripts/update-data.mjs` generates `notifications` from high-signal disaster `criticalEvents`.
  - `app/index.html` renders a phone-visible `通知規則` panel.
  - `scripts/send-notifications.mjs` sends queued events through Gmail from GitHub Actions.
...
- Verified notification delivery:
  ...
  - Ritsu confirmed the Gmail notification was received.
  - Latest data after verification records `deliveryStatus: "gmail-sent"` and `deliveredAt`.
- Operational workflow:
  - Normal scheduled runs are UTC `00:00` and `12:00`, currently Taiwan / Japan time `09:00` and `21:00`.
  - Manual refresh is GitHub Actions -> `Update dashboard data` -> `Run workflow` on `main`.
  - Do not use `Re-run jobs` for old runs when `main` has moved; start a new `Run workflow` instead.
- Secret setup:
  - Required repository secrets: `GMAIL_USER`, `GMAIL_APP_PASSWORD`, `ALERT_EMAIL_TO`.
...
2026-05-03: Notification Layer v1 is complete and deployed.

- Branch: `codex/notification-layer-final`
- Merged PR: `https://github.com/RitsuTsao/hakkoda-safety-dashboard/pull/12`
- Merge commit: `c602196 Merge pull request #12 from RitsuTsao/codex/notification-layer-final`
...
- Scope:
  - Add `notifications` to `app/data.json`.
  - Add a phone-visible `通知規則` panel.
  - Keep notification candidates limited to high-signal disaster `criticalEvents`.
  - Classify immediate candidates for 三區域震度5以上地震, 青森 / 岩手津波注意報以上, 三區域土砂災害警戒情報以上, and 熊傷人.
  - Keep yellow events, routine advisories, and trip-operation interruptions out of notification candidates.
  - Add Gmail SMTP delivery through `scripts/send-notifications.mjs`.
  - Run Gmail sender after scheduled data update and before auto-commit.
  - Bump service worker cache to `hakkoda-safety-v14`.
- Validation:
  - `node --check scripts/update-data.mjs`
  - `node --check scripts/send-notifications.mjs`
  - `env -u GMAIL_USER -u GMAIL_APP_PASSWORD -u ALERT_EMAIL_TO node scripts/send-notifications.mjs`
  - `node scripts/update-data.mjs`
  - GitHub Actions manual run sent 1 Gmail notification after the secret-name typos were fixed.
  - Ritsu confirmed the live page and Gmail receipt.
...
- Current version: Version 1.0.0 data pipeline, including JMA XML updater v1, Visual Map v1.2 event filtering, Bear Info v1.1 summaries, Precision Sources v1 operation extraction, and Notification Layer v1 rules.
...
- Generates `notifications` from high-signal disaster `criticalEvents`.
- Notification rules:
  - 24-hour digest: red events only, same event should not repeat more than once every 24 hours when delivery is enabled.
  - Immediate class: 三區域震度5以上地震, 青森 / 岩手津波注意報以上, 三區域土砂災害警戒情報以上, 熊傷人.
  - Routine yellow events, operation status, and routine advisories should not notify.
  - `scripts/update-data.mjs` only queues notifications; `scripts/send-notifications.mjs` marks them delivered after Gmail succeeds.
- Sends Gmail notifications through GitHub Actions when repository secrets are present.
- Writes successful delivery state back to `app/data.json`, including `deliveryStatus: "gmail-sent"` and `deliveredAt`.
...
- GitHub Pages cannot provide true background push by itself. Notification delivery requires an external channel or backend.
...
- Notification Layer v1 depends on GitHub Actions cadence and Gmail SMTP; it is not a true instant push channel.
- Same-event notification suppression depends on `app/data.json` being committed successfully after Gmail delivery.
...
3. Data Source follow-up after v2.
...
5. Offline Emergency Mode follow-up refinements.
...
6. Notification Layer follow-up.
   - Keep the current high-signal disaster-only behavior unless Ritsu explicitly accepts more noise.
   - If using GitHub Actions for delivery, consider increasing schedule frequency during the trip, while still treating official apps as the true immediate alert layer.
   - After the trip, rotate or delete the Gmail App Password and repository secret if notifications are no longer needed.

## Suggested Prompt For New Project

Use this prompt in the new Project conversation:

> Please continue development of `RitsuTsao/hakkoda-safety-dashboard`. First read `docs/handoff.md`, then inspect the current repo state. The app is deployed as a GitHub Pages PWA at `https://ritsutsao.github.io/hakkoda-safety-dashboard/app/index.html`. Version 1.0.0 is complete as of 2026-05-03: scheduled data refresh, focused official links, offline emergency mode, visual-map filtering, bear summaries, precision source panels, and Gmail notification delivery are all live. Continue from the current implementation; do not restart from scratch. The next likely task is live QA, notification cadence tuning, source-link/parser refinement, or conservative event-noise reduction after real evidence appears.
