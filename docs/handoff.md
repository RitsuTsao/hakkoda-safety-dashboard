...
2026-05-03: Notification Layer v1 is in local development on branch `codex/notification-layer-v1`.

- Scope:
  - Add a `notifications` layer to `app/data.json`, generated by `scripts/update-data.mjs`.
  - Render a phone-visible `通知規則` panel in `app/index.html`.
  - Keep notification candidates limited to high-signal disaster `criticalEvents`.
  - Classify immediate notification candidates for:
    - 三區域震度5以上地震
    - 青森 / 岩手津波注意報以上
    - 三區域土砂災害警戒情報以上
    - 熊傷人
  ...
  - Keep Gmail delivery inactive until GitHub Secrets are configured.
  - Bump service worker cache to `hakkoda-safety-v14`.
- Current local behavior:
  - Live-data candidate is currently `岩手 熊被害`, marked `immediate` with reason `熊傷人`.
  - Yellow `岩手山` volcano / ashfall event remains visible in the Visual Map but is not a notification candidate.
  - The panel explains that GitHub Pages itself cannot background-push; Gmail is the selected external channel.
  - `.github/workflows/update-data.yml` now runs `scripts/send-notifications.mjs` after data update and before auto-commit.
  - If Gmail secrets are missing, the send step logs a safe skip and does not mark the queued notification as delivered.
- GitHub Secrets needed before live Gmail delivery:
  - `GMAIL_USER`: sending Gmail address.
  - `GMAIL_APP_PASSWORD`: Google App Password, not the normal Google password.
  - `ALERT_EMAIL_TO`: recipient address, usually the same Gmail address or a Notion Mail linked address.
- User setup status:
  - Ritsu reported on 2026-05-03 that all three GitHub Secrets have been configured.
- Manual setup notes:
  - Ritsu must create / paste the Google App Password personally; do not ask them to paste it into chat.
  - After travel, rotate or delete the App Password and remove the GitHub Secret if no longer needed.
- PR / merge status:
  - Do not push or open PR until Ritsu confirms the final local behavior and agrees to the one-PR flow.
...
- Current version: JMA XML updater v1 plus Visual Map v1.1 event generation and Notification Layer v1 rules.
...
- Notification Layer v1 creates notification candidates but has not yet connected an external delivery channel.
...
5. Notification Layer follow-up.
   - Complete Gmail App Password and GitHub Secrets setup.
...
- `app/service-worker.js` cache version is current enough to force PWA refresh after UI changes. Current local UI cache version after Notification Layer v1: `hakkoda-safety-v14`.
